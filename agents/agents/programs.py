...
class ProgramsAgent(MiddlewareIntegrationMixin):
    ...
    def __init__(self):
        ...
        # v11: Initialize coaching asset selector (strategy/opportunity techniques)
        ...
        if COACHING_ASSETS_AVAILABLE:
            try:
                ...
                mw_logger.info("[Programs] Coaching assets enabled (strategy techniques)")
            except Exception as e:
                mw_logger.warning(f"Coaching asset init failed (non-fatal): {e}")

    async def _select_programs_technique(
        self,
        matching_type: str = "opportunity_matching",
    ) -> Optional[Dict[str, Any]]:
        """
        Select the best coaching technique for program matching.

        Uses strategy techniques for program/opportunity matching.
        ...
        try:
            context = {
                "event_type": f"programs_{matching_type}",
                "keywords": ["programs", "summer", "opportunities", "matching"],
                "tags": ["strategy", "programs", "opportunities"],
            }
            ...
            mw_logger.warning(f"Programs technique selection failed (non-fatal): {e}")
            return None
