...
class TestMiddlewareStack:
    ...
    @pytest.mark.asyncio
    async def test_wrap_agent_context_manager(self):
        """Test wrap_agent as async context manager."""
        from middleware.stack import MiddlewareStack, AgentContext
        
        # Initialize without clients to avoid mock issues
        middleware = MiddlewareStack()
        
        # wrap_agent is async, returns context manager
    ...
    @pytest.mark.asyncio
    async def test_wrap_agent_auto_session_id(self):
        """Test wrap_agent generates session_id if not provided."""
        from middleware.stack import MiddlewareStack
        
        middleware = MiddlewareStack()
        
        ctx_manager = await middleware.wrap_agent("ec_agent", "profile-123")
        async with ctx_manager as ctx:
...
class TestAgentExecutionContext:
    ...
    @pytest.mark.asyncio
    async def test_context_starts_metrics(self):
        """Test context manager starts metrics collection."""
        from middleware.stack import MiddlewareStack
        
        # Use middleware without clients to avoid async mock issues
        middleware = MiddlewareStack()
    ...
    async def test_context_cleans_up_on_exit(self):
        ...
        # After exit, current context should be cleared
        assert middleware._current_context is None
